...
async function fetchJSON(path: string) {
  try {
    ...
    console.warn(`[property-detail] Supabase returned ${res.status}, trying local backup...`);
  } catch (err) {
    console.warn(`[property-detail] Supabase unreachable: ${err instanceof Error ? err.message : err}`);
  }
  ...
  console.log(`[property-detail] Served from local backup: ${path}`);
