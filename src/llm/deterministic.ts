...
const AVAILABILITY_ASK_RE =
  /(дали[^.!?\n]{0,40}(?:достапен|достапна|достапно|слободен|слободна|слободно|слободна|слободно|продаден|продадена|издаден|издадена|на продажба|на prodazba|постои|го имате уште|ја имате уште)|достапен\s+ли\s+е|достапна\s+ли\s+е|слободен\s+ли\s+е|слободна\s+ли\s+е|продаден\s+ли\s+е|издаден\s+ли\s+е|сеуште\s+(?:ли\s+)?(?:е\s+)?(?:достапен|достапна|слободен|на продажба)|(?:е|е\s+ли)\s+(?:слободен|слободна|слободно|достапен|достапна|достапно)|(?:го|ја)\s+имате\s+(?:ли\s+)?(?:уште|сеуште)|(?:go|ja)\s+imate\s+(?:li\s+)?(?:uste|seuste)|daa?[il][il][^.!?\n]{0,40}(?:dostapen|dostapna|dostapno|sloboden|slobodna|slobodno|prodaden|prodadena|izdaden|izdadena|na prodazba|postoi|go imate uste|ja imate uste)|dostapen\s+li\s+e|dostapna\s+li\s+e|sloboden\s+li\s+e|slobodna\s+li\s+e|prodaden\s+li\s+e|izdaden\s+li\s+e|seuste\s+(?:li\s+)?(?:e\s+)?(?:dostapen|dostapna|sloboden|na prodazba)|(?:e|e\s+li)\s+(?:sloboden|slobodna|slobodno|dostapen|dostapna|dostapno))/iu;
...
const EXACT_ADDRESS_RE =
  /(?:пото?чно|точно|tochno|tocno|potocno|pokazete|покажете|која|koja|којашто|koja)\s+(?:е\s+(?:му\s+|mu\s+)?|mu\s+е\s+|му\s+е\s+|да\s+е\s+)?(?:точната|точн|tochnata|tocnata|tochna|tocna)?\s*(?:адреса|adresa|улица|ulica|локацијата|lokacijata|локација|lokacija)|(?:на\s+која|na\s+koja)\s+(?:адреса|adresa|улица|ulica)|(?:адресата|adresata|улицата|ulicata|локацијата|lokacijata)\s+(?:ќе|ke|да)\s+(?:ја|ja)\s+(?:доби|dobij|знам|znam|кажи|kazi)|(?:кажи|kazi)\s+(?:ми\s+|mi\s+)?(?:ја|ja)\s+(?:адресата|adresata|улицата|ulicata|точно|tochno|tocno|каде\s+точно)|(?:кажи|kazi)\s+(?:ја|ja)\s+(?:адресата|adresata|улицата|ulicata)|точно\s+(?:каде|kade)\s+(?:е|e)|каде\s+точно|kade\s+tocno|kade\s+tochno|(?:каде|kade)\s+(?:е\s+|e\s+)?(?:точната|tochnata|tocnata|точна|tochna|tocna|прецизната|preciznata)\s*(?:адреса|adresa|улица|ulica|локација|lokacija)|(?:moram|mora|treba|морам|мора|треба|сакам|sakam)\s+(?:(?:да|da)\s+)?(?:знам|znam)\s+(?:каде|kade)\s+(?:е|e|се\s+нао[гѓ]а|se\s+naogja|се|se)|(?:морам|мора|moram|mora|треба|treba)\s+(?:(?:да|da)\s+)?(?:знам|znam)\s+(?:каде|kade)\s+е|za\s+da\s+se\s+odlu(?:cam|čam)|за\s+да\s+се\s+одлу(?:чам|кам)|(?:која|koja)\s+(?:му\s+|mu\s+)?(?:е|e)\s+(?:точната|tochnata|tocnata|точн|tochn|tocn|прецизната|preciznata)\s*(?:локација|lokacija|адреса|adresa|улица|ulica)|(?:која|koja)\s+(?:му\s+|mu\s+)?(?:е|e)\s+(?:адресата|adresata|улицата|ulicata|локацијата|lokacijata)\s*[-–]?\s*(?:точна|tochna|tocna|точната|tochnata|tocnata)|(?:која|koja)\s+(?:е|e)\s+(?:адресата|adresata|улицата|ulicata|локацијата|lokacijata)|(?:која|koja)\s+(?:е|e)\s+(?:адреса|adresa)\s+(?:на|na)\s+(?:стан|stanot|sopstvenik)|(?:дај|daj)\s+(?:ми|mi)\s+(?:ја|ja)\s+(?:адресата|adresata|улицата|ulicata|локацијата|lokacijata)|(?:дад(?:и|ете)|dadi(?:te)?)\s+(?:ми|mi)\s+(?:ја|ja)?\s*(?:адресата|adresata|улицата|ulicata|локацијата|lokacijata)|(?:адресата|adresata)\s*(?:[-–]\s*)?(?:точна|tochna|tocna|точната|tochnata|tocnata)|(?:точната|tochnata|tocnata|точна|tochna|tocna|прецизната|preciznata)\s+(?:адреса|adresa|улица|ulica|локација|lokacija|адресата|adresata|локацијата|lokacijata)|(?:адресата|adresata)\s+(?:ќе|ke|да)\s+(?:ја|ja)\s+(?:доби|dobij|dobam|знам|znam)\s+(?:кога|koga)/iu;
