...
const AVAILABILITY_RE3 = new RegExp(
  ...
  // NOTE (the [09:25] transcript): the земат/нудите arm here is the one that
  // matched "зематр провизија за посета?" (zematr = земат + r-typo). The
  // FEE-context veto at the top of detectAvailabilityAsk now guards it —
  // do NOT widen the arm; tighten it if it over-fires again.
...
export function detectAvailabilityAsk(text: string): boolean {
  ...
  // arm of the sweep regexes read "зематр…?" as "is it taken?". A
