...
const SPEC: GenerationKey[] = [
  ...
  // in the database, but the current availability AND price are the owner's
  // truth, so she will contact the owner and confirm. NO question, NO price,
  // NO fee, NO property-type word (the client knows which property he means).
  {
    ...
    sources: [
      ...
      'Сеуште постои во нашата база на податоци. Ќе морам да го контактирам сопственикот и да ја потврдам достапноста и цената.',
    ],
    count: 12,
    instructions: 'Клиентот прашува дали конкретен имот (чиј Евидентен број го знае од огласот) е сеуште достапен — одговори дека би требало да е достапен / сеуште постои во базата, но моменталната достапност и цената ги потврдува сопственикот, па ќе го контактираш и ќе ја провериш состојбата. БЕЗ прашање, БЕЗ износи, БЕЗ надомест, БЕЗ спомнување на стан/куќа/деловен простор, БЕЗ Евидентен броеви. Природен, смирувачки тон. Варирај го воведот (Би требало / Според нашата база / Имотот постои / …) и изразот за контакт (ќе го контактирам / ќе стапам во контакт / ќе се поврзам / ќе го исконтактирам).',
    required: [/(?:достапен|достапна|достапно|постои|база|слободен|слободна|активен|активна|на располагање|понуда)/iu, /(?:сопственик|провер|потврд|контактирам|исконтактирам|поврзам|стапам во контакт)/iu],
